...
let citations = [
  ...
  let nouvelleCitation = document.getElementById('nouveau');
  let tailleTableauCitations = citations.length;
  let choix;
  ...
  nouvelleCitation.addEventListener('click', start); 

  function start(){
    choix = getRandomInt(tailleTableauCitations); // choix de la citation 
    citation.innerText =  citations[choix][0] ;
    auteur.innerText = citations[choix][1]
  }

  function getRandomInt(x) {
    return Math.floor(Math.random() * tailleTableauCitations);
  }
